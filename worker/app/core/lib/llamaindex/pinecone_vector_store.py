...
class CustomPineconeVectorStore(PineconeVectorStore):
    ...
    metadata_transformer: Optional[
        Callable[[Dict[str, Any], BaseNode], Dict[str, Any]]
    ] = Field(default=None, description="Custom metadata transformation function")
    ...
    enable_hierarchical_filtering: bool = Field(
        default=True, description="Enable hierarchical-aware filtering"
    )
    ...
    def _update_progress(self, message: str, current: int, total: int) -> None:
        ...
        logger.info(
            f"Pinecone Progress: {message} - {current}/{total} ({percentage:.1f}%)"
        )
    ...
    def _transform_metadata(self, node: BaseNode) -> Dict[str, Any]:
        ...
        if "hierarchical_level" in node.metadata:
            ...
            enhanced_metadata["importance_score"] = node.metadata.get(
        ...
        llmsherpa_keys = ["llmsherpa_tag", "llmsherpa_block_class", "llmsherpa_level"]
    ...
    def add(
        ...
        **add_kwargs: Any,
        ...
        self._update_progress("Starting node addition to Pinecone", 0, len(nodes))
        ...
        for i, node in enumerate(nodes):
            ...
            self._update_progress(
                f"Processing node {i+1}/{len(nodes)}", i + 1, len(nodes)
            )
        ...
        self._update_progress(
            f"Successfully added {len(nodes)} nodes to Pinecone", len(nodes), len(nodes)
        )
    ...
    def _build_hierarchical_filter(self, filters: MetadataFilters) -> MetadataFilters:
        ...
        if filters.filters:
            ...
            all_filters = list(hierarchical_filters)

        return MetadataFilters(filters=all_filters, condition=filters.condition)
    ...
    def query(self, query: VectorStoreQuery, **kwargs: Any) -> VectorStoreQueryResult:
        ...
        if result.nodes:
            result.nodes = self._post_process_hierarchical_results(list(result.nodes))
    ...
    def _post_process_hierarchical_results(
        ...
        def sort_key(node: BaseNode) -> tuple[Any, ...]:
        ...
        try:
            ...
            try:
                ...
                if ids_to_delete:
                    ...
                    self._update_progress(
                        f"Deleted {len(ids_to_delete)} nodes for {ref_doc_id}", 1, 1
                    )
            except Exception as fallback_error:
                logger.error(
                    f"Fallback delete also failed for {ref_doc_id}: {fallback_error}"
                )
                raise
    ...
    def delete_document(
        ...
        """
        Delete all vectors associated with a document using document_id prefix.
        ...
        try:
            ...
            if progress_callback:
                progress_callback("Starting document deletion", 0, 1)

            # First, try to query for vectors with document_id in metadata
            logger.info(f"Querying for vectors with document_id: {document_id}")

            if progress_callback:
                progress_callback("Querying for vectors to delete", 0, 1)
            ...
            if query_response.matches:
                ...
                logger.info(
                    f"Found {len(vector_ids)} vectors to delete for document "
                    f"{document_id}"
                )
            ...
            if vector_ids:
                ...
                if progress_callback:
                    progress_callback(
                        f"Deleting {len(vector_ids)} vectors in batches",
                        0,
                        len(vector_ids),
                    )
                ...
                for i in range(0, len(vector_ids), batch_size):
                    ...
                    logger.info(
                        f"Deleting batch {i//batch_size + 1}: {len(batch)} vectors"
                    )
                    logger.debug(f"Batch vector IDs: {batch}")

                    # Delete the batch
                    self._pinecone_index.delete(ids=batch)
                    deleted_count += len(batch)

                    # Update progress
                    if progress_callback:
                        progress_callback(
                            f"Deleted {deleted_count}/{len(vector_ids)} vectors",
                            deleted_count,
                            len(vector_ids),
                        )
                ...
                logger.info(
                    f"Successfully deleted {deleted_count} vectors for document "
                    f"{document_id}"
                )
            ...
            # Final progress update
            if progress_callback:
                progress_callback(f"Document deletion completed", 1, 1)
            ...
            if progress_callback:
                progress_callback(f"Deletion failed: {str(e)}", 0, 1)
