...
class ContentTypeAwareSplitter(SentenceSplitter):
    ...
    def get_chunk_config_for_content(
        self, content_type: str, level: int
    ) -> Dict[str, Any]:
        """
        ...
        return {
            "chunk_size": level_rules.get(
                "chunk_size", base_rules.get("chunk_size", self.chunk_size)
            ),
            "chunk_overlap": level_rules.get(
                "chunk_overlap", base_rules.get("chunk_overlap", self.chunk_overlap)
            ),
        }
...
class CustomHierarchicalNodeParser(HierarchicalNodeParser):
    ...
    content_type_config: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Configuration for different content types"
    )
    ...
    def from_defaults(
        ...
        # Create node parser IDs and map if not provided
        ...
        # Only create parsers if node_parser_map is empty
        if not node_parser_map:
            for chunk_size, node_parser_id in zip(chunk_sizes, node_parser_ids):
                node_parser_map[node_parser_id] = cast(
                    NodeParser,
                    ContentTypeAwareSplitter(
    ...
    def _add_enhanced_relationships(
        ...
        child_node.metadata["parent_level"] = parent_node.metadata.get(
            "hierarchical_level", 0
        )
        child_node.metadata["child_level"] = child_node.metadata.get(
    ...
    def _recursively_get_nodes_from_nodes(
        ...
        self._update_progress(f"Processing level {level} nodes", 0, len(nodes))
        ...
        for i, node in enumerate(nodes):
            ...
            self._update_progress(
                f"Processing level {level} node {i+1}/{len(nodes)}", i + 1, len(nodes)
            )
            ...
            # Process the node
            cur_sub_nodes = node_parser.get_nodes_from_documents([cast(Document, node)])
    ...
    def get_nodes_from_documents(
        ...
        **kwargs: Any,
        ...
        self._update_progress("Starting document processing", 0, len(documents))
