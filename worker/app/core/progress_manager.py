...
class ProgressManager:
    ...
    def start_stage(self, stage_name: str) -> None:
        ...
        self._send_progress_update(f"Started {stage_name}", 0, 100, stage_name)
    ...
    def update_stage_progress(self, message: str, current: int, total: int) -> None:
        ...
        self._send_progress_update(message, current, total, self.current_stage)
    ...
    def complete_stage(self, stage_name: str) -> None:
        ...
        duration = datetime.now() - self.stage_start_times.get(
            stage_name, self.start_time
        )
    ...
    def _send_progress_update(
        self, message: str, current: int, total: int, stage: str
    ) -> None:
        """
        ...
        try:
            ...
            response = requests.post(
                f"{self.backend_url}/worker/status", json=progress_data, timeout=10
            )
    ...
    def report_status_to_backend(
        ...
        try:
            ...
            payload = {
                ...
                "message": f"Document {self.operation_type} {status}",
            ...
            logger.info(
                f"Reporting status to backend: {status} for document {self.document_id}"
            )

            # Send to backend via REST API
            response = requests.post(
                f"{self.backend_url}/worker/status", json=payload, timeout=10
            )

            if response.status_code == 200:
                logger.info(
                    f"Successfully reported status to backend for document {self.document_id}"
                )
            else:
                logger.warning(
                    f"Backend returned status code {response.status_code} for document {self.document_id}"
                )

        except Exception as e:
            logger.error(
                f"Failed to report status to backend for document {self.document_id}: {str(e)}"
            )
            # Don't raise here as this shouldn't fail the main task

    def send_final_progress_update(
        self,
        status: str,
        chunks: Optional[List[Any]] = None,
        error: Optional[str] = None,
        ...
        try:
            ...
            logger.info(
                f"Final progress update sent for document {self.document_id} with status: {status}"
            )

        except Exception as e:
            logger.error(
                f"Failed to send final progress update for document {self.document_id}: {str(e)}"
            )
            # Don't raise here as this shouldn't fail the main task
